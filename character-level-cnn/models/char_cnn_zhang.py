...
class CharCNNZhang(object):
    ...
    def _build_model(self):
        ...
        """
        # Input layer
        inputs = Input(shape=(self.input_size,), name='inputs', dtype='int64')
        # Embedding layers
        x = Embedding(self.alphabet_size + 1, self.embedding_size,
    ...
    def test(self, testing_inputs, testing_labels, batch_size):
        ...
        metrics = self.model.evaluate(
            testing_inputs, testing_labels, batch_size=batch_size, verbose=1)
        print(metrics)
        # self.model.predict(testing_inputs, batch_size=batch_size, verbose=1)
        """

        prediction = self.model.predict(testing_inputs)
        predicted_label = np.argmax(prediction, axis=1).flatten()
        testing_labels = np.argmax(testing_labels, axis=1).flatten()

        print("Test Accuracy: ", accuracy_score(testing_labels, predicted_label))
        print("Test Precision: ", precision_score(testing_labels, predicted_label, average=None))
        print("Test Recall: ", recall_score(testing_labels, predicted_label, average=None))
        print("Test F1-Score: ", f1_score(testing_labels, predicted_label, average=None))