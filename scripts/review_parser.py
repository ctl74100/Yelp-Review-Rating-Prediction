...
def parse_restaurant_ids(num_of_restaurants=5000):
    """
...
def parse_review(restaurant_ids, total_reviews_limit=100000):
    """
    ...
    with open('./dataset/review.json', 'r', encoding='utf-8') as file:
        with open('./dataset/parsed_review.csv', 'w', encoding='utf-8', newline='') as csvfile:
            ...
            star_stat = [0] * 5
            star_limit = [total_reviews_limit * x for x in [0.15, 0.20, 0.3, 0.20, 0.15]]
            print(star_limit)
...
if __name__ == '__main__':
    filepath_ = __file__[:-34] + '/dataset/parsed_review.csv'
